...
class CreateHTML(CreatePage):
    ...
    def write_results(self, work_dict):
        ...
        texts += "</p>"
        texts += "<p><strong>Word Counts by Funder:</strong><br>"
        for k, v in sorted(self.funder_words.items(), key=lambda x: x[1], reverse=True):
            texts += "<strong>{}:</strong> {:,}<br>".format(k, v)
        texts += "</p>"
...
def cmd():
    ...
    parser.add_argument('--gitpage', default="https://github.com/{}".format(os.environ['TRAVIS_REPO_SLUG']),
